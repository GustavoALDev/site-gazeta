-- CreateTable: Nova tabela unificada para Top Categories (sem foreign key inicialmente)
...
-- AddForeignKey: Nova foreign key para top_categories_configs (só se a tabela users existir)
...
SET @sql = IF(@categories_exists > 0,
...
-- DropForeignKey: Remover foreign keys antigas (se existirem)
SET @table_exists = (SELECT COUNT(*) FROM information_schema.tables WHERE table_schema = DATABASE() AND table_name = 'destaque_category_relations');
SET @sql = IF(@table_exists > 0,
    CONCAT('ALTER TABLE `destaque_category_relations` DROP FOREIGN KEY IF EXISTS `destaque_category_relations_destaque_config_id_fkey`;
            ALTER TABLE `destaque_category_relations` DROP FOREIGN KEY IF EXISTS `destaque_category_relations_category_id_fkey`'),
    'SELECT 1');
SET @sql = IF(@table_exists > 0, @sql, 'SELECT 1');
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
SET @table_exists = (SELECT COUNT(*) FROM information_schema.tables WHERE table_schema = DATABASE() AND table_name = 'destaque_configs');
SET @sql = IF(@table_exists > 0,
    'ALTER TABLE `destaque_configs` DROP FOREIGN KEY IF EXISTS `destaque_configs_created_by_fkey`',
    'SELECT 1');
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
SET @table_exists = (SELECT COUNT(*) FROM information_schema.tables WHERE table_schema = DATABASE() AND table_name = 'top_gazeta_category_relations');
SET @sql = IF(@table_exists > 0,
    CONCAT('ALTER TABLE `top_gazeta_category_relations` DROP FOREIGN KEY IF EXISTS `top_gazeta_category_relations_top_gazeta_config_id_fkey`;
            ALTER TABLE `top_gazeta_category_relations` DROP FOREIGN KEY IF EXISTS `top_gazeta_category_relations_category_id_fkey`'),
    'SELECT 1');
SET @sql = IF(@table_exists > 0, @sql, 'SELECT 1');
PREPARE stmt FROM @sql;
EXECUTE stmt;
...
SET @table_exists = (SELECT COUNT(*) FROM information_schema.tables WHERE table_schema = DATABASE() AND table_name = 'top_gazeta_configs');
SET @sql = IF(@table_exists > 0,
    'ALTER TABLE `top_gazeta_configs` DROP FOREIGN KEY IF EXISTS `top_gazeta_configs_created_by_fkey`',
    'SELECT 1');
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;

-- DropTable: Remover tabelas antigas (se existirem)
