...
export class SubmenuFormComponent {
  ...
  submitForm(event?: Event): void {
    ...
    console.log('🔄 Submetendo formulário (submenu):', this.menuForm.value);
    console.log('🔄 Categorias selecionadas (submenu):', this.selectedCategories());
    console.log('🔄 Form válido (submenu):', this.menuForm.valid);
    ...
    this.menuService.create(menuData).subscribe({
      ...
      error: (err) => {
        ...
        this.alertService.error('Erro', 'Erro ao criar submenu. Tente novamente.');
      }
    });
  }
  ...
  private createMultipleCategoryMenus(): void {
    ...
    console.log('🔄 Criando múltiplos menus de categorias (submenu):', {
    ...
    this.menuService.createCategoryMenus(menusData, parentId).subscribe({
      next: (menus) => {
        console.log('✅ Menus criados com sucesso (submenu):', menus);
        this.isLoading.set(false);
      ...
      error: (err) => {
        ...
        this.alertService.error('Erro', 'Erro ao criar submenus. Tente novamente.');
      }
    });
  }
